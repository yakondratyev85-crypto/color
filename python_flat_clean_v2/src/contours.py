...
def beautify_contours(
    ...
    """Second-pass contour beautification for noisy masks.

    This layer avoids leaving contour problems untouched: high-jitter or
    staircase-heavy paths get resampled, straightened, and organically smoothed
    once more, while tiny invalid loops are dropped. It deliberately reuses the
    same hybrid fitting primitives so straight architectural runs remain straight
    and corners are not globally blurred.
    """
    metrics = {"beautified_contours": 0, "dropped_contours": 0}
    result: list[dict] = []
    clean_mode = mode == "clean"
    for contour in contours:
        points = contour["points"].astype(np.float32)
        if len(points) < 3 or float(contour.get("area", 0)) < 8:
            metrics["dropped_contours"] += 1
            continue
        staircase = _contour_staircase(points)
        jitter = _contour_jitter(points)
        too_dense = len(points) > max(80, int(np.sqrt(float(contour.get("area", 1))) * 5.5))
        needs_beauty = clean_mode or staircase > 0.62 or jitter > 0.22 or too_dense
        if needs_beauty:
            spacing = 5.0 if clean_mode else 4.0
            points = resample_contour(points, spacing=spacing)
            if preserve_straight:
                points = straighten_near_line_segments(points, min_run=3 if clean_mode else 4)
            extra = max(1, min(3, int(round(smoothing_strength / 2))))
            points = smooth_organic_curves(points, iterations=extra, preserve_corners=preserve_corners)
            metrics["beautified_contours"] += 1
        updated = dict(contour)
        updated["points"] = points
        result.append(updated)
    return result, metrics


def _contour_staircase(points: np.ndarray) -> float:
    if len(points) < 2:
        return 0.0
    axis = 0
    total = 0
...
def _contour_jitter(points: np.ndarray) -> float:
    if len(points) < 4:
        return 0.0
    values = []
    for idx in range(len(points)):
        a = points[(idx - 1) % len(points)]
        b = points[idx]
        c = points[(idx + 1) % len(points)]
