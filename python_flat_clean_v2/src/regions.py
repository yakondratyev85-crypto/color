...
class Region:
    ...
    rag_neighbors: dict[int, RagEdge] = field(default_factory=dict)
...
def build_region_adjacency_graph(
    ...
    """Build and attach a Region Adjacency Graph (RAG) with merge scores.

    The graph is used by cleanup to merge micro/sliver regions into the most
    plausible neighbor instead of simply picking an arbitrary nearby color.
    Scores intentionally balance color closeness, shared boundary length,
    neighbor size, shape compatibility, and an important-edge penalty.
    """
    ...
    for region in regions:
        ...
        source_edge_penalty = _edge_penalty(region, important_edges)
        for neighbor_id, shared in region.neighbors.items():
            ...
            color_similarity = 1.0 / (1.0 + color_distance / (32.0 + merge_strength * 55.0))
            shared_boundary_length = shared / max_shared
            neighbor_area_score = min(1.0, np.log1p(neighbor.area) / np.log1p(max(min_area * 20, 2)))
            shape_compatibility = max(0.0, min(1.0, neighbor.compactness * 2.3 + neighbor.bbox_fill * 0.45))
            important_edge_penalty = min(1.0, source_edge_penalty * 0.7 + _edge_penalty(neighbor, important_edges) * 0.3)
            merge_score = (
                color_similarity * 0.35
                + shared_boundary_length * 0.30
                + neighbor_area_score * 0.20
                + shape_compatibility * 0.10
                - important_edge_penalty * 0.05
            )
            allowed_distance = 76.0 + merge_strength * 80.0
            merge_allowed = color_distance <= allowed_distance or region.area < min_area * (1.2 + merge_strength)
            edge = RagEdge(
                source=region.id,
                target=neighbor.id,
                shared_boundary=int(shared),
                color_distance=color_distance,
                neighbor_area=int(neighbor.area),
                merge_allowed=merge_allowed,
                merge_score=float(merge_score),
            )
            region.rag_neighbors[neighbor.id] = edge
...
def merge_bad_regions(
    ...
    metrics = {"merged_regions": 0, "micro_regions": 0, "sliver_regions": 0, "rag_edges": 0, "bad_regions": 0}

    for _pass in range(max_passes):
        regions, _component_map = connected_regions(result)
        by_id = {region.id: region for region in regions}
        rag_edges = build_region_adjacency_graph(regions, palette, min_area, merge_strength, important_edges)
        metrics["rag_edges"] = max(metrics.get("rag_edges", 0), len(rag_edges))
        candidates = [region for region in regions if _bad_region_kind(region, min_area)]
        metrics["bad_regions"] = max(metrics.get("bad_regions", 0), len(candidates))
        if not candidates:
            break
        changed = False
...
def _choose_best_neighbor(
    ...
    best_id = None
    best_score = -999.0
    if not region.rag_neighbors:
        build_region_adjacency_graph(list(by_id.values()), palette, min_area, merge_strength, important_edges)
    for neighbor_id, edge in region.rag_neighbors.items():
        if neighbor_id not in by_id or not edge.merge_allowed:
            continue
        if edge.merge_score > best_score:
            best_score = edge.merge_score
            best_id = neighbor_id
    return best_id
